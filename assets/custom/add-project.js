...
//Dropdown Material
async function dropdownMaterial(selectId) {
  try {
    const response = await fetch(`${product_API}/get`, {
      method: "GET",
      headers: {
    ...
    materialSelect.innerHTML = `<option selected disabled>Select Material</option>`;
...
window.onload = async () => {
  await dropdownMaterial("material-select");
};
// =======================================================
// =======================================================
...
document.getElementById("add_project_form").addEventListener("submit", async function (event) {
  ...
  try {
    ...
    try{
        status_popup( ((c1) ? "Project Created <br> Successfully!" : "Please try <br> again later"), (c1) );
...
function rntFormData(){
  ...
  // Attach general project details
  ...
  rows.forEach((row, index) => {
    ...
    formData.append(`paymentDetails[${index}][mbNumber]`, cells[6]?.querySelector("input")?.value || "");
