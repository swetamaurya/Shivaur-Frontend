...
async function editDateLoad() {
  ...
  try{
    ...
    console.log(document.getElementById("prodMaterialSelectOption"))
    console.log(r1?.material?._id);
    console.log(document.getElementById("prodMaterialSelectOption").value="6778e5c518bf29d9eb05ed11")

    try{

      document.getElementById("orderCategorySelectOption").value = r1?.orderCategory;
      if(r1?.orderCategory=="Client"){
        document.getElementById("clientDivCol").classList.remove("d-none");
        document.getElementById("clintSelectOption").value = r1?.client?._id;
      } else {
        document.getElementById("projectDivCol").classList.remove("d-none");
        document.getElementById("projSelectOption").value = r1?.project?._id;
      }
      document.getElementById("status").value = r1?.status;
      document.getElementById("saleClintSelectOption").value = r1?.saleTo?._id;
      document.getElementById("customerOrderRef").value = r1?.customerOrderRef;
      document.getElementById("orderRef").value = r1?.orderRef;
      document.getElementById("orderDate").value = r1?.orderDate;
      document.getElementById("complicationDate").value = r1?.complitionDate;
      document.getElementById("prodMaterialSelectOption").value = r1?.material?._id;
      let ooo1 = getMaterialSelectedObject(r1?.material?._id);
      document.getElementById("qtySpan").innerText = ooo1?.quantity;
      document.getElementById("qty").setAttribute("max",ooo1?.quantity);

      document.getElementById("qty").value = r1?.quantity;
      document.getElementById("unitCost").value = r1?.unit;
      document.getElementById("delieveryDateToConsumer").value = r1?.deliveryDateToCustomer;
      document.getElementById("price").value = r1?.price;
      document.getElementById("dues").value = r1?.dues;
      document.getElementById("netPayable").value = r1?.netAmount;
      document.getElementById("ttlRcvdAmt").value = r1?.totalReceivedAmount;
      document.getElementById("ttlDdcnAmt").value = r1?.totalDeductionAmount;
      document.getElementById("description").value = r1?.description;
    
      let r3 = r1?.paymentDetails;
      for(let i = 0; i<r3.length; i++){
        let r4 = r3[i];
        const row = document.querySelectorAll(".tbodyonepayment .paymentData");
        
        let cell = row[i].querySelectorAll("td");
        
        try{
          let cell1Input = cell[1]?.querySelector("input");
          if (cell1Input) cell1Input.value = r4?.date;
          
          let cell2Input = cell[2]?.querySelector("select");
          if (cell2Input) cell2Input.value = r4?.terms;
          
          let cell3Input = cell[3]?.querySelector("input");
          if (cell3Input) cell3Input.value = r4?.amountDetails;
          
          let cell4Input = cell[4]?.querySelector("select");
          if (cell4Input) cell4Input.value = r4?.status;
          
          let cell5Select = cell[5]?.querySelector("select");
          if (cell5Select) cell5Select.value = r4?.transactionType;
          
          let cell6Input = cell[6]?.querySelector("input");
          if (cell6Input) cell6Input.value = r4?.amount;

          let cell7Input = cell[7]?.querySelector("input");
          if (cell7Input) cell7Input.value = r4?.paymentDate;

          if(r4?.status=="Paid"){
            cell5Select.removeAttribute("disabled");
            cell7Input.removeAttribute("disabled");
...
formSale.addEventListener("submit", async function(event){
  ...
  try{
    ...
    try{
      status_popup( ((c1) ? "Sale Updated <br> Successfully!" : "Please try <br> again later"), (c1) );
      setTimeout(function(){
