...
async function dropdownForAddTask(){
  ...
  project_select_option.value = pId;
  // =========================================================
  // =========================================================
...
addTaskForm.addEventListener('submit',async(event)=>{
  ...
  try {
    ...
    try{
      status_popup( ((c1) ? "Task Assigned <br> Successfully!" : "Please try <br> again later"), (c1) );
      
      let abmohit = await batchFetchProducts();
      let ab2mohit = await batchUpdateProducts(abmohit);
      setTimeout(function(){
        window.location.href = `project-view.html?id=${pId}`; 
      },(Number(document.getElementById("b1b1").innerText)*20000));
    } catch (error){
